...
const FEATURES = [
  {
    ...
    description: 'Three simulated CNC machines each stream one AI4I dataset row per second — temperature, RPM, torque, and tool wear — over WebSockets.',
    bullets: ['One-second playback', 'Three independent streams', 'CSV export'],
  },
  {
    icon: BrainCircuit,
    accent: '#BF5AF2',
    accentSubtle: 'rgba(191, 90, 242, 0.12)',
    title: 'Manual-Grounded AI Diagnosis',
    description: 'LangGraph combines the latest PostgreSQL telemetry with relevant passages retrieved from three Pinecone-indexed CNC manuals.',
    bullets: ['Three parallel retrievers', 'PDF manual grounding', 'Gemini synthesis'],
  },
  {
    icon: AlertTriangle,
    accent: '#FF9F0A',
    ...
    description: 'The simulator surfaces labeled AI4I failures plus torque and tool-wear warnings, then preserves the sensor window that led to the event.',
    bullets: ['Five labeled failure types', 'Threshold warnings', 'Failure context replay'],
  },
  {
    icon: BarChart3,
    accent: '#30D158',
    ...
    description: 'Compare CNC-01, CNC-02, and CNC-03 as each progresses through a different, non-overlapping slice of the dataset.',
    bullets: ['Product quality filters', 'Per-machine history', 'Live status changes'],
  },
  {
    icon: Database,
    accent: '#64D2FF',
    accentSubtle: 'rgba(100, 210, 255, 0.12)',
    title: 'Dataset Playback Simulator',
    description: 'Start, pause, or reset the playback. Each machine stops when it reaches its first labeled failure, recreating a clear maintenance event.',
    bullets: ['Synthetic AI4I 2020 data', 'Stop on labeled failure', 'Persistent telemetry history'],
  },
  {
    icon: Shield,
    accent: '#FF453A',
    accentSubtle: 'rgba(255, 69, 58, 0.12)',
    title: 'See How the AI Worked',
    description: 'Open the diagnosis inspector to see the SQL telemetry window, generated search queries, retrieved manual context, and final synthesis.',
    bullets: ['Visible LangGraph flow', 'Retrieved context', 'Operator-focused output'],
  },
];
...
const STEPS = [
  {
    ...
    icon: Server,
    accent: '#0A84FF',
    title: 'Replay Three Machines',
    description: 'Three non-overlapping AI4I dataset slices stream one row per second and are stored in PostgreSQL.',
  },
  {
    number: '02',
    icon: AlertTriangle,
    accent: '#FF9F0A',
    title: 'Stop at Failure',
    description: 'Each stream raises an alert and stops when it reaches a labeled machine-failure row.',
  },
  {
    number: '03',
    icon: Sparkles,
    accent: '#BF5AF2',
    title: 'Diagnose with Manuals',
    description: 'LangGraph cross-references the final telemetry window with three CNC manuals indexed in Pinecone.',
  },
];
    ...
    <div className="landing-gradient-bg text-white font-sans antialiased overflow-x-hidden">
      ...
      <nav style={{ position:'fixed',top:0,left:0,right:0,zIndex:50,padding:'16px 32px',display:'flex',alignItems:'center',justifyContent:'space-between',backdropFilter:'blur(20px)',WebkitBackdropFilter:'blur(20px)',background:'rgba(10,10,15,0.72)',borderBottom:'1px solid rgba(255,255,255,0.06)' }} className="animate-fade-in">
        <div style={{ display:'flex',alignItems:'center',gap:'10px' }}>
          ...
          <div>
            <span style={{ fontWeight:800,fontSize:'16px',letterSpacing:'-0.02em',background:'linear-gradient(135deg,#fff 0%,#64D2FF 100%)',WebkitBackgroundClip:'text',WebkitTextFillColor:'transparent',backgroundClip:'text' }}>CNC Failure Lab</span>
            <div style={{ fontSize:'10px',color:'rgba(255,255,255,0.45)',fontWeight:600,letterSpacing:'0.06em',textTransform:'uppercase',marginTop:'1px' }}>Predictive Maintenance Demo</div>
          </div>
        </div>
        <div style={{ display:'flex',alignItems:'center',gap:'12px' }}>
          ...
          <button onClick={onLaunch} className="landing-cta-btn" style={{ padding:'9px 20px',fontSize:'13px' }} id="nav-launch-btn">
            Open Live Demo<ChevronRight size={14} />
          </button>
        </div>
      </nav>
      ...
      <section style={{ position:'relative',minHeight:'100vh',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',textAlign:'center',padding:'120px 24px 80px',overflow:'hidden' }}>
        ...
        <div className="animate-fade-up" style={{ display:'inline-flex',alignItems:'center',gap:'8px',padding:'6px 16px',borderRadius:'9999px',background:'rgba(48,209,88,0.10)',border:'1px solid rgba(48,209,88,0.25)',marginBottom:'28px',animationDelay:'0.1s' }}>
          ...
          <span style={{ fontSize:'11px',fontWeight:700,color:'#30D158',letterSpacing:'0.06em',textTransform:'uppercase' }}>Interactive Portfolio Demo · Synthetic AI4I Data</span>
        </div>
        ...
        <h1 className="animate-fade-up" style={{ fontSize:'clamp(2.8rem,7vw,5.5rem)',fontWeight:900,letterSpacing:'-0.04em',lineHeight:1.05,maxWidth:'900px',marginBottom:'24px',animationDelay:'0.2s' }}>
          <span className="gradient-text">Watch CNC Machines Fail.</span><br />
          <span style={{ color:'rgba(255,255,255,0.95)' }}>Then Diagnose Why.</span>
        </h1>
        ...
        <p className="animate-fade-up" style={{ fontSize:'clamp(1rem,2vw,1.2rem)',color:'rgba(255,255,255,0.55)',maxWidth:'600px',lineHeight:1.7,marginBottom:'44px',animationDelay:'0.3s' }}>
          Three simulated CNC machines replay the synthetic AI4I 2020 dataset one row per second.
          When a labeled failure appears, the stream stops and a{' '}
          <span style={{ color:'#64D2FF',fontWeight:600 }}>manual-grounded LangGraph workflow</span>{' '}
          explains the telemetry and recommends corrective actions.
        </p>
        ...
        <div className="animate-fade-up" style={{ display:'flex',gap:'16px',flexWrap:'wrap',justifyContent:'center',animationDelay:'0.4s' }}>
          <button onClick={onLaunch} className="landing-cta-btn" id="hero-launch-btn">
            <Play size={16} fill="white" strokeWidth={0} />Open Live Demo<ArrowRight size={16} />
          </button>
        ...
        <div className="animate-fade-up" style={{ marginTop:'72px',position:'relative',maxWidth:'900px',width:'100%',animationDelay:'0.55s' }}>
          ...
          <div style={{ position:'relative',background:'rgba(255,255,255,0.04)',border:'1px solid rgba(255,255,255,0.10)',borderRadius:'24px',padding:'24px',backdropFilter:'blur(20px)',overflow:'hidden' }}>
            ...
            <div style={{ display:'flex',alignItems:'center',gap:'8px',marginBottom:'20px',paddingBottom:'16px',borderBottom:'1px solid rgba(255,255,255,0.06)' }}>
              ...
              <span style={{ fontSize:'12px',color:'rgba(255,255,255,0.35)',marginLeft:'8px',fontFamily:'monospace' }}>CNC Failure Lab — Three-Machine Simulation</span>
            </div>
            {/* Metric cards */}
            <div style={{ display:'grid',gridTemplateColumns:'repeat(5,1fr)',gap:'12px',marginBottom:'16px' }}>
              {[
                { label:'Machines',value:'3',color:'#BF5AF2' },
                { label:'Running',value:'2',color:'#30D158' },
                { label:'Warnings',value:'0',color:'#FF9F0A' },
                { label:'Failed',value:'1',color:'#FF453A' },
                { label:'Open Alerts',value:'1',color:'#64D2FF' },
              ].map((m) => (
            ...
            <div style={{ display:'flex',justifyContent:'flex-end',marginTop:'12px' }}>
              <div style={{ display:'inline-flex',alignItems:'center',gap:'6px',padding:'6px 14px',borderRadius:'9999px',background:'rgba(191,90,242,0.15)',border:'1px solid rgba(191,90,242,0.3)',fontSize:'11px',fontWeight:700,color:'#BF5AF2' }}>
                <Sparkles size={11} />Manual-Grounded Diagnosis Ready
              </div>
            </div>
          </div>
        </div>
      ...
      <section style={{ padding:'64px 24px',maxWidth:'900px',margin:'0 auto' }}>
        <div style={{ display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'32px',textAlign:'center' }}>
          {[
            { value:'3', label:'Simulated CNC Machines', sublabel:'Independent dataset slices' },
            { value:'1 sec', label:'Telemetry Interval', sublabel:'Per machine via WebSocket' },
            { value:'3', label:'Indexed CNC Manuals', sublabel:'Retrieved through Pinecone' },
      ...
      <section style={{ padding:'100px 24px',maxWidth:'1100px',margin:'0 auto' }} ref={addRevealRef}>
        <div style={{ textAlign:'center',marginBottom:'64px' }}>
          ...
          <h2 style={{ fontSize:'clamp(1.8rem,4vw,2.8rem)',fontWeight:800,letterSpacing:'-0.03em',color:'rgba(255,255,255,0.95)' }}>
            From synthetic telemetry to a<br /><span className="gradient-text-blue">manual-grounded diagnosis</span>
          </h2>
        </div>
      ...
      <section style={{ padding:'100px 24px',maxWidth:'1200px',margin:'0 auto' }}>
        <div style={{ textAlign:'center',marginBottom:'64px' }} ref={addRevealRef}>
          ...
          <h2 style={{ fontSize:'clamp(1.8rem,4vw,2.8rem)',fontWeight:800,letterSpacing:'-0.03em',color:'rgba(255,255,255,0.95)' }}>
            Explore the complete<br /><span className="gradient-text">failure-to-diagnosis workflow</span>
          </h2>
        </div>
      ...
      <section style={{ padding:'80px 24px',maxWidth:'900px',margin:'0 auto',textAlign:'center' }} ref={addRevealRef}>
        ...
        <h2 style={{ fontSize:'clamp(1.5rem,3vw,2.2rem)',fontWeight:800,letterSpacing:'-0.03em',color:'rgba(255,255,255,0.90)',marginBottom:'40px' }}>The stack behind the working demo</h2>
      ...
      <section style={{ padding:'100px 24px',textAlign:'center',position:'relative',overflow:'hidden' }} ref={addRevealRef}>
        ...
        <h2 style={{ fontSize:'clamp(2rem,5vw,3.5rem)',fontWeight:900,letterSpacing:'-0.04em',color:'rgba(255,255,255,0.95)',marginBottom:'20px',lineHeight:1.1 }}>
          Follow a machine from<br /><span className="gradient-text">healthy operation to failure</span>
        </h2>
        <p style={{ fontSize:'16px',color:'rgba(255,255,255,0.50)',maxWidth:'480px',margin:'0 auto 44px',lineHeight:1.7 }}>
          Start the dataset playback, watch each machine stop at a labeled failure, then ask the AI to compare its final sensor readings with the indexed manuals.
        </p>
        <div style={{ display:'flex',gap:'16px',justifyContent:'center',flexWrap:'wrap' }}>
          <button onClick={onLaunch} className="landing-cta-btn" id="bottom-launch-btn" style={{ padding:'16px 36px',fontSize:'16px' }}>
            <Play size={18} fill="white" strokeWidth={0} />Open Live Demo<ArrowRight size={18} />
          </button>
      ...
      <footer style={{ borderTop:'1px solid rgba(255,255,255,0.06)',padding:'32px 24px',display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:'16px',maxWidth:'1200px',margin:'0 auto' }}>
        <div style={{ display:'flex',alignItems:'center',gap:'10px' }}>
          ...
          <span style={{ fontSize:'13px',fontWeight:600,color:'rgba(255,255,255,0.50)' }}>CNC Failure Lab · Predictive Maintenance Demo</span>
        </div>
